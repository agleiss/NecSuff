...
/* Author's documented example: Swedish lung-cancer cohort (Nilsson et al, 2001).
   Expressed as a 4-row frequency table so the full cohort is preserved exactly
   while staying compact; %necsuff's freq= parameter carries the cell counts. */
data lungca;
	input x y count;
	datalines;
0 0 8120
...
;
run;
%necsuff(data=lungca, y=y, x=x, freq=count);
